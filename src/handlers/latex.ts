...
export default class ExportAsLaTeX {
    ...
    advancedFillText(text: string, originalText: string, x: number, y: number, angleOrNull: number, c: CanvasRenderingContext2D) {
        if (text.replace(' ', '').length > 0) {
            let nodeParams = '';
            // x and y start off as the center of the text, but will be moved to one side of the box when angleOrNull != null
            if (angleOrNull != null) {
                const width = this.measureText(text, c).width;
